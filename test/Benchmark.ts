...
const AFFORDABLE_CREATE_BATCH_GAS_LIMIT = 27000000;
...
describe("Benchmark", function(){
  ...
  it("Should REDEEM voucher with many schedules successfully", async function(){
    ...
    const gasEstimation = await voucher.connect(account1).redeem.estimateGas(0, totalAmount);
    console.log(`Gas estimation REDEEM ${gasEstimation}`);

    // assertions
    await expect(voucher.connect(account1).redeem(0, totalAmount)).to.not.be.reverted;
  });

  it("Should CREATE BATCH voucher with estimate gas successfully", async function(){
    const {
      voucher,
      erc20Token,
      nftCollection,
    ...
    for (const item of data) {
      ...
      const schedules = await mockVestingSchedules(item.scheduleNumber);  
      let vesting = {
      ...
      // gas estimation
      const gasEstimation = await voucher.connect(account1).createBatch.estimateGas(batch);
      console.log(`Gas estimation CREATE BATCH ${gasEstimation}`);

      // assertions
      await expect(voucher.connect(account1).createBatch(batch, {gasLimit: AFFORDABLE_CREATE_BATCH_GAS_LIMIT})).to.not.be.reverted;
    }

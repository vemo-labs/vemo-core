...
contract DataRegistry is AccessControl, IDynamic {
  ...
  function writeBatch(address requester, address nftCollection, uint256 fromId, uint256 len, bytes32 key, bytes calldata value)
    public onlyRole(WRITER_ROLE)    
    returns (bool)
  {
    require(requester != address(0), "Requester must be live account");
    ...
    for (uint i = 0; i < len; i++) {
      _registry[nftCollection][fromId + i][key] = value;
      emit Write(requester, nftCollection, fromId + i, key, value);
    }    
